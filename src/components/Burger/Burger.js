...
const burger = (props) => {
    ...
    let ingredientsArray = Object.keys(props.ingredients) // creates an array from object, where each value in the array is a key of the object
        .map(ingKey => { // map the array, with each item in the array returning....
            return [...Array(props.ingredients[ingKey].count)].map((_, i) => { // ... a nested array the length of the value of each key (if there's 2 salads, the nested array with have a length of two) -- underscore as first arg means we don't care about it, we justwant the index (2nd arg)
